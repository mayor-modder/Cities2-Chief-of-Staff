...
- When generating a city report, refresh Save Investigator first through CityAdvisor's `analyze` or report-producing MCP tools; do not rely on an existing Save Investigator output directory unless the user explicitly asks for a stale/offline read.
